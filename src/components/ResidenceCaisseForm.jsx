...
import { notifyResidenceCaisseEntry } from '../lib/ntfy'
import { uploadResidencePhoto } from '../lib/storage'
import { compressImage } from '../lib/imageCompress'
import { RC_OPERATION_TYPES, RC_PAYMENT_MODES, RC_CATEGORIES } from '../lib/residence'
...
export default function ResidenceCaisseForm() {
  ...
  async function loadSuggestions() {
    const [{ data: bonRow }, { data: rows }, { data: resaClients }] = await Promise.all([
      supabase.from('residence_caisse').select('bon_number').order('bon_number', { ascending: false }).limit(1),
      supabase.from('residence_caisse').select('beneficiary, client_name, cheque_bank').order('created_at', { ascending: false }).limit(500),
      supabase.from('residence_clients').select('name').order('name'),
    ])
    ...
    setClients(dedupe([...(rows?.map((r) => r.client_name) ?? []), ...(resaClients?.map((c) => c.name) ?? [])]))
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (draft.photo_file) {
        ...
        photo_url = await uploadResidencePhoto(compressed, `caisse-${bonNumber}`)
      }
      ...
      notifyResidenceCaisseEntry(data)
      ...
      setSuccess(`Bon caisse résidence n° ${bonNumber} enregistré.`)
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <Field label="Type d'opération" required>
        <select value={draft.operation_type} onChange={(e) => update('operation_type', e.target.value)} className={inputClass}>
          {RC_OPERATION_TYPES.map((o) => <option key={o} value={o}>{o}</option>)}
        </select>
      </Field>

      <Field label="Motif / Libellé" required>
        <input type="text" value={draft.description} onChange={(e) => update('description', e.target.value)} className={inputClass} placeholder="ex : Encaissement réservation, Femme de ménage, Réparation chauffe-eau" required />
      </Field>
      ...
      <Field label="Fournisseur / Bénéficiaire">
        <input type="text" list="rc-beneficiaries-list" value={draft.beneficiary} onChange={(e) => update('beneficiary', e.target.value)} className={inputClass} autoComplete="off" placeholder="qui reçoit ou donne l'argent" />
        <datalist id="rc-beneficiaries-list">
          {beneficiaries.map((b) => <option key={b} value={b} />)}
        </datalist>
      </Field>

      <Field label="Client">
        <input type="text" list="rc-clients-list" value={draft.client_name} onChange={(e) => update('client_name', e.target.value)} className={inputClass} autoComplete="off" placeholder="optionnel" />
        <datalist id="rc-clients-list">
      ...
      {isCheque && (
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <Field label="Banque" required>
            <input type="text" list="rc-banks-list" value={draft.cheque_bank} onChange={(e) => update('cheque_bank', e.target.value)} className={inputClass} autoComplete="off" required />
            <datalist id="rc-banks-list">
